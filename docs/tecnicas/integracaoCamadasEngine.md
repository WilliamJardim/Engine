...
A camada integradora é responsável por integrar a Engine de lógica/fisica com a de renderização, repassando para a camada de renderização os objetos que a camada de lógica/fisica está criando ou manipulando. Além disso, a camada integradora também é reponsável por prover informações necessárias para elas, como informações de entrada do teclado e mouse. A camada integradora também cria as seguintes variáveis:
  ...
  - `LimiteFPS`: um número, que define a taxa de frames por segundo(FPS) que o renderizador vai usar. Essa taxa é usada na função `loop_principal`, na linha 608, do arquivo `IntegradorCamadas.ts`. Você pode ver o código completo do arquivo `IntegradorCamadas.ts` que será citado abaixo caso queira ver os detalhes.
...
Links para o código da camada integradora(do arquivo RenderizadorCena.ts):
  ...
  - Código mais recente da camada integradora: [Camada integradora - do último commit, ou seja, o commit mais recente](../../src/engine/IntegradorCamadas.ts)

**Nota: a camada integradora atualmente é o arquivo `src/engine/IntegradorCamadas.ts`**
